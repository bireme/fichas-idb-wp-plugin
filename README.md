...
0. [Download](https://github.com/bireme/fichas-idb-wp-plugin/archive/master.zip) the Fichas plugin for Wordpress;
0. Unzip the plugin below the `wp-content/plugins` folder of your Wordpress instance and rename it to `fichas`;
...
Go to `Settings` in the administration panel (dashboard) and click on the newly created `Fichas` item.
* `Plugin page` is mandatory and is set to `fichas` by default. It defines the URL of the search interface page;
